 ...
 * Copyright (c) 2023 Da'Jour J. Christophe. All rights reserved.
 */
...
keypair_t *keypair_new(void *key,
                       void *val, const size_t key_size,
                                  const size_t val_size)
{
  if (key == NULL || key_size == 0)
  {
    fprintf(stderr, "%s\n", "keypair_new(): null key pointer or zero key size");
    exit(EXIT_FAILURE);
  }
  ...
  if (keypair == NULL)
  {
    fprintf(stderr, "%s\n", "keypair_new(): memory error");
    exit(EXIT_FAILURE);
  }

  keypair->key = calloc(1, key_size);
  if (keypair->key == NULL)
  {
    fprintf(stderr, "%s\n", "keypair_new(): memory error");
    exit(EXIT_FAILURE);
  ...
  if (val != NULL && val_size != 0)
  {
    ...
    keypair->val_size = val_size;
    memcpy(keypair->val, val, val_size);
  }

  return keypair;
}

void keypair_destroy(keypair_t *keypair)
{
  if (keypair->key != NULL)
  {
    free(keypair->key);
...
int keypair_compare(const void *a, const void *b, void *udata)
{
  const keypair_t *kpa = a;
  const keypair_t *kpb = b;

  if (kpa->key_size != kpb->key_size)
  {
    return 0;
  }

  return memcmp(kpa->key, kpb->key, kpa->key_size || kpb->key_size);
}

uint64_t keypair_hash(const void *item, uint64_t seed0, uint64_t seed1)
{
  const keypair_t *keypair = item;
  return hashmap_sip(keypair->key, keypair->key_size, seed0, seed1);
}

void map_set(map_t *map, void *key, void *val, const size_t key_size, const size_t val_size)
{
  hashmap_set(map, keypair_new(key, val, key_size, val_size));
}

void *map_get(map_t *map, void *key, const size_t key_size, size_t *n)
{
  keypair_t *result = NULL;
  result = hashmap_get(map, &(keypair_t){.key=key,.val=NULL,.key_size=key_size,.val_size=0});
  if (n)
  {
    *n = result->val_size;
  }
...
void map_set_str_int(map_t *map, char *key, int x)
{
  map_set(map, key, &x, strlen(key), sizeof(int));
}

int *map_get_str_int(map_t *map, char *key)
{
  return map_get(map, key, strlen(key), NULL);
}
...
void symbol_table_add(symbol_table_t *table, char *symbol, int value)
{
  map_set_str_int(table, symbol, value);
}

int *symbol_table_get(symbol_table_t *table, char *symbol)
{
  map_get_str_int(table, symbol);
}
...
int main(void)
{
  ...
  symbol_table_add(table, "a", 5);
  symbol_table_add(table, "b", 7);
  symbol_table_add(table, "c", 9);

  printf("%d\n", *symbol_table_get(table, "a"));
  printf("%d\n", *symbol_table_get(table, "b"));
  printf("%d\n", *symbol_table_get(table, "c"));
